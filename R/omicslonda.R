...
#' @export
omicslonda = function(se_object = NULL,
                      n.perm = 500,
                      ...
                      prefix = "Test")
{
  ...
  points = points[which(points >= points.min &
                          points <= points.max)]
  ...
  if (fit.method == "ssgaussian")
  {
    ...
    model = tryCatch({
      curveFitting(formula = Count ~ Time,
                   df,
                   fit.method = "ssgaussian",
                   points)
    },  error = function(err) {
  ...
  output.details = list(
    feature = rep(text, length(interval.start)),
    ...
    testStat = stat,
    testStat.abs = abs(stat),
    testStat.sign = sign(stat),
    dominant = dominant,
